...
watch(() => filter.value, () => {
	setTimeout(() => {
		...
		router.push(`/content-frame/${collection.value}`)
	}, 900)
}, { deep: true })
...
<template>
	...
	<private-view v-else v-model:splitView="splitView" :split-view-min-width="310" :title="title" hide-all-navigation>
		...
		<template #headline>
			<v-breadcrumb
				v-if="collectionInfo.meta && collectionInfo.meta.singleton === true"
				:items="[{ name: t('content'), to: '/content-frame' }]"
			/>
